...
const ActionLink = ({ href, className, children }: { href: string; className?: string; children: React.ReactNode }) =>
  href.startsWith("/") ? <Link to={href} className={className}>{children}</Link> : <a href={href} className={className}>{children}</a>;
...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Honeypot: a hidden field real users never fill; bots almost always do.
    // (No timing heuristic — browser autofill legitimately submits in under a few seconds.)
          ...
          <div className="grid lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-3">
              <div className="mb-8">
                ...
                <p className="text-muted-foreground text-sm">We read every message and reply within one business day.</p>
              </div>
              {submitted ? (
